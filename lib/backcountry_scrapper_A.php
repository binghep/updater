...
class backcountry_scrapper_A{
	...
	function __construct($bc_url){//or encode($url)
	    ...
	    $pos1=strpos($scrapped_website, 'BC.product.skusCollection = $.parseJSON(');
	...
	public function build_php_object($scrapped_website,$pos1){
	    $removed_before=substr($scrapped_website,$pos1+strlen('BC.product.skusCollection = $.parseJSON(')+1);
	    ...
	    return !is_null($this->php_object) && json_last_error()===0 ;
	}
	/**
