...
class backcountry_scrapper_B{
	...
	function __construct($bc_url){//or encode($url)
	    ...
	    $scrapped_website = $this->curl($bc_url);  // Executing our curl function to scrape the webpage http://www.example.com and return the results into the $scrapped_website variable
	    echo (strlen($scrapped_website));
		//--------------get this js block---------------------------
	    ...
	    $pos1=strpos($scrapped_website, '<script type="text/javascript">BC.product =');
	    // var_dump($pos1);
	    if ($pos1!==false){
		    $result=$this->build_php_object($scrapped_website,$pos1);
	    	if ($result===true){
...
public function build_php_object($scrapped_website,$pos1){
		$removed_before=substr($scrapped_website,$pos1+strlen('<script type="text/javascript">BC.product =')+1);
	    $pos2=strpos($removed_before, "$.publish('pdp.domReady");//); BC.product.title
	    // var_dump($pos2);
	   	if ($pos2===false){
	    	return false;
	    }
	    ...
	    if (is_null($this->php_object) || json_last_error()!==0 || !property_exists($this->php_object, "skusCollection")){
	    	return false;
	    }

	    // var_dump($this->php_object);
    	$this->php_object=$this->php_object->skusCollection;
    	return !empty($this->php_object) && !is_null($this->php_object);
}
	/**
