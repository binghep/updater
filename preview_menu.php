...
foreach ($categories as $category){
	...
	if (!is_null($filter_strings[$category->getId()])){
		echo "<a  href='preview.php?cat_id=",$category->getId(),"'>",$category->getName(),"</a>";
	}else{
	...
	foreach ($second_level_cat_ids as $id){
		...
		if ($cat!==false){
			if (!is_null($filter_strings[$cat->getId()])){
				echo "<div><a  href='preview.php?cat_id=",$cat->getId(),"'>",$cat->getName(),"</a></div>";
			}else{
...
// 			$url=substr($url,$i+6);
// 			// var_dump($url);
// 			echo '<a href="preview.php?cat_id='.$cat->getId().'">'.$cat->getName().'</a>';
