...
<body>
<span class="right">
	<a href="preview_menu.php">Back To Menu</a>
</span>
...
class datafeedr_preview{
			...
			echo 'View ',$num_products_per_page,' per page.<br>';
				...
				for ($i=1; $i <= $total_page_number ; $i++) { 
					...
					if ($i==$page_number){
						...
						echo "<a href='preview.php?cat_id=",$this->magento_category_id,"&p=",$i,"'>  ",$i,"  </a>";
					}
				}
				echo '<br>';
			if ($debug){
	...
	*/
	...
	function init_filter_strings(){
		...
		echo '</table>';
		if (is_null($strings)){
			echo 'category id is not in $filter_strings in preview_config.php. Exiting...';
	...
	*/
	...
	function test_process($products)
	{
		...
		foreach($products as $product) {
			  ...
			  echo '<span style="display:none">'.$product['sku'].'</span>';
